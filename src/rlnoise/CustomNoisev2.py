            ...
            for moment in range(no_noise_circ_rep.shape[0]):
                ...
                for qubit in range(no_noise_circ_rep.shape[1]):
                    ...
                    if len(gate_idx) != 0:
                        ...
                        gate=self.rep.index2gate[int(gate_idx)]
                        if gate is not gates.CZ:
                            noisy_circuit.add(gate(q=qubit,theta=theta))
                            epsilon_idx=self.rep.gate2epsilonidx[gate]
                            epsilon_type=[i for i in self.rep.epsilon2index if self.rep.epsilon2index[i]==epsilon_idx]
                            epsilon_value=params.getfloat('noise',str(epsilon_type[0]))
                            #print('epsilon value: ',epsilon_value)                           
                            if epsilon_value!=0:
                                noisy_circuit.add(gate(q=qubit,theta=theta*epsilon_value))
                            if self.Depol_on_gate is gate:
                                noisy_circuit.add(DepolarizingChannel(lam=self.lam,q=qubit))
                            #elif self.Therm_on_gate is gate: #watch out if not muted on line 30 will be added 
                            #    noisy_circuit.add(ThermalRelaxationChannel(t_1=self.t1,t_2=self.t2,time=self.time,q=qubit))
                            elif self.Damping_on_gate is gate:
                                noisy_circuit.add(ResetChannel(q=qubit,p0=self.p0,p1=0.))
                            ...
                            if count==-1:
                                ...
                                elif self.Damping_on_gate is gates.CZ:
                                    noisy_circuit.add(ResetChannel(q=qubit,p0=self.p0,p1=0.))
                                    noisy_circuit.add(ResetChannel(q=count,p0=self.p0,p1=0.))
            '''
