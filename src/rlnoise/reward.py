...
def trace_distance(dm1: np.ndarray, dm2: np.ndarray) -> float:
    ...
    where |A| = sqrt(Aâ€  A) is the matrix absolute value.
...
class RewardFunction:
    ...
    def _get_transform_function(self, function: str, alpha: float) -> Callable:
        ...
        """
        transform_map = {
            # Add small offset to avoid log(0) or division by zero
            "log": lambda x: -np.log(alpha * x + 1e-9),
            "linear": lambda x: -alpha * x,
            "inverted": lambda x: 1.0 / (alpha * x + 1e-9),
