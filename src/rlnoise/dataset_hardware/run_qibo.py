...
if method == 'ST':
    ...
    for circ in qibo_training_circuits:
        ...
        model = ClassicalShadows(circuit=circ,shadow_size=1000)

        model.get_classical_shadow(backend=qibolab)
        rho = model.shadow_state_reconstruction()

        circ.density_matrix = True
        rho_exact = numpy.execute_circuit(circ).state()
        log.info(circ.draw())
        log.info([circ, rho_exact, rho])
        rhos.append([circ, rho_exact, rho])


    np.save(benchmark_circ_path+'density_matrices_cs.npy',rhos)
